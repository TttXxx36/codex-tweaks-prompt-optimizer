...
1. 选择协议，填写用户自己的 HTTPS API 地址、Key 和模型名称。OpenAI 兼容服务通常应填写以 `/v1` 结尾的地址；若根地址返回 HTML 应用页，功能包只会有限改试一次对应的 `/v1` API 路径。
2. 可用“获取模型”探测 `/models`；成功后可从“选择已获取模型”下拉框直接选择，也仍可手动填写模型。
