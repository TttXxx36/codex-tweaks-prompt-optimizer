...
- 在宿主设置适配器可用时，于个人设置页提供“提示词优化”配置。
- 在 Composer 的模型选择器附近提供“优化”主按钮及设置菜单；宿主设置适配器不可用时，菜单会打开包内完整设置面板，不影响优化功能启动。
...
2. 选择协议，填写用户自己的 HTTPS API 地址、Key 和模型名称。OpenAI 兼容服务通常应填写以 `/v1` 结尾的地址；若根地址返回 HTML 应用页，功能包只会有限改试一次对应的 `/v1` API 路径。
3. 可用“获取模型”探测 `/models`；成功后可从“选择已获取模型”下拉框直接选择，也仍可手动填写模型。
4. 修改优化指令后点击“保存配置”。草稿配置可用于“测试连接”和“获取模型”，但不会因为测试而落盘。
5. 选择历史保留数量 `0/5/10/20/50`。设置为 `0` 时不持久化历史，但当前页面生命周期仍保留最近一次恢复快照。
6. 卸载前可在“卸载前清理数据”区域执行“清理包数据”，它会清除 Key、历史和已保存 Provider 配置。
