...
- Understanding: 用户反馈最新版本的“优化”按钮跑到 Codex 页面左上角，不在 Composer 内；需要重新检查上一版固定层实现。本次已获授权推送到 GitHub，不发布 Release。
...
- Result: 先加入 2 个失败回归用例，再完成修复；`npm test` 39/39 通过；`npm run check` 和 `git diff --check` 通过。代码已通过 GitHub API 同步到 `main` 提交 `e278c6ab4cad04d317f8461d61ab21e82b8d22bc`；未创建新 Release。
