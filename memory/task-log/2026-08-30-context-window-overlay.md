...
- Understanding: 截图显示“优化”按钮仍在 `5.6 Luna` 上方，且背景信息窗口打开后没有出现在 Composer；需要分析并同时修复两项问题。未授权本次推送或发布。
...
- Result: 新增回归测试先失败后通过；覆盖不改写宿主工具栏、左侧/垂直居中几何、背景信息窗口关闭回退；`npm test` 37/37 通过；`npm run check` 通过；`git diff --check` 通过。尚未推送或发布。
