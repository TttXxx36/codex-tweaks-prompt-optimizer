...
- Root cause: Composer 的候选输入元素、Composer 外壳、背景信息窗口和模型选择器是异步且分层挂载的；只在输入值变化时渲染，或把模型选择器直接当作最终锚点，会造成空输入隐藏和按钮落在模型区域上方。
- Fix: 将按钮生命周期绑定到 Composer 外壳；在模型选择器之前优先识别背景信息窗口的语义标记，并插入其父容器左侧；用模型选择器和发送按钮作为降级锚点，按钮自身保持同一行垂直居中。
- Prevention: 为空输入、Codex/Work/ChatGPT 三种 Composer、重复注入、节点替换和背景信息窗口锚点分别保留 DOM 回归用例；禁止使用发送按钮作为模型选择器或背景信息窗口存在时的首选锚点。
