...
export function activate({ root, onCleanup, api: _api, ui, node } = {}) {
  ...
  const positionComposerButton = (entry, { previousAnchor = null, phase = "position" } = {}) => {
    ...
    const position = getComposerButtonPosition(anchorRect, combinedButtonRect, viewportSize(), 36);
    if (!position) {
  ...
  const renderPanel = () => {
    ...
    const content = element(doc, "div", { className: `ctpo-panel-content ctpo-panel-body-${panelState.kind}` });
  ...
  const renderPreviewContent = (panel, actions, panelState) => {
    ...
    const splitContainer = element(doc, "div", { className: "ctpo-panel-preview ctpo-panel-split-layout" });
  ...
  const renderClarifyContent = (panel, actions, panelState) => {
    panel.append(element(doc, "p", { className: "ctpo-hint" }, [`第 ${panelState.round}/3 轮需求拷问与澄清（Grill-me 风格）。勾选推荐选项或直接填写补充：`]));
    ...
    if (panelState.busy) {
      panel.append(element(doc, "div", { className: "ctpo-status" }, ["正在深度分析提示词并生成澄清问题……"]));
    } else if (panelState.questions.length) {
      const questionsList = element(doc, "div", { className: "ctpo-question-list" });
      panelState.questions.forEach((qObj, index) => {
        ...
        const questionCard = element(doc, "div", { className: "ctpo-question-card" });

        const typeBadge = element(doc, "span", { className: "ctpo-question-badge" }, [isMulti ? "多选" : "单选"]);
        const qTitle = element(doc, "div", { className: "ctpo-question-title" }, [
          element(doc, "span", { className: "ctpo-question-index" }, [`${index + 1}.`]),
          element(doc, "span", { className: "ctpo-question-text" }, [qText]),
          typeBadge,
        ]);
        questionCard.append(qTitle);

        if (options.length > 0) {
          const optionsContainer = element(doc, "div", { className: "ctpo-options-grid" });
          options.forEach((opt, optIdx) => {
            ...
            const optInput = element(doc, "input", {
              type: inputType,
              name: `ctpo-q-${index}`,
            ...
            const optCard = element(doc, "label", {
              htmlFor: `ctpo-q-${index}-opt-${optIdx}`,
              className: "ctpo-option-card",
            });

            const checkIndicator = element(doc, "span", { className: `ctpo-indicator ctpo-indicator-${inputType}` });
            const optContent = element(doc, "div", { className: "ctpo-option-content" });

            const cleanLabel = optLabel.replace(/^\((?:推荐|Recommended)\)\s*/i, "");
            const labelRow = element(doc, "div", { className: "ctpo-option-label" }, [
              isRec ? element(doc, "span", { className: "ctpo-rec-badge" }, ["推荐"]) : null,
              element(doc, "span", {}, [cleanLabel]),
            ]);
            optContent.append(labelRow);

            if (optDesc) {
              const descRow = element(doc, "div", { className: "ctpo-option-desc" }, [optDesc]);
        ...
        const customInput = element(doc, "textarea", {
  ...
  const runClarifyRound = async (panelState) => {
    ...
    try {
      ...
      if (panelState.ready && typeof response.prompt === "string" && response.prompt.trim()) {
        showPreview({
          original: panelState.original,
          result: response.prompt.trim(),
          clarifications: panelState.answers,
          mode: "clarify",
  ...
  const collectClarificationAnswers = (panelState) => {
    return panelState.questions.map((qObj, index) => {
      ...
      const checkedInputs = panelHost.querySelectorAll(`input[name="ctpo-q-${index}"]:checked`);
      ...
      const customInput = panelHost.querySelector(`textarea[data-ctpo-question-index="${index}"]`);
